...
setwd('/Users/jingjinghuang/Documents/Courses/ML_in_R/materials/Session1')
list.files()

library(readr)

df <- read_csv("df_session1.20200811.csv")
df = data.frame(df)
...
# summary(mod) is an object with itw own components e.g. $coefficients
...
mod = randomForest(geneB ~ age, data = train_set, ntree = 1, maxnodes = 3)
